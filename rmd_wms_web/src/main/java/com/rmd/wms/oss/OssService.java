...
public class OssService {
	 ...
	 * @throws IOException
	 */
	public OssFile Ossupload(MultipartFile file) throws IOException {
		return upload(file, createDefaultObjectMetadata(file));
	}

	protected OssFile upload(MultipartFile file, ObjectMetadata metadata) throws IOException {
		String fileName = fileNameBuilder.build(file.getOriginalFilename());
//		metadata.setContentType("image/jpeg");
		OssFile ossFile = ossUpload(fileName, file.getInputStream(), metadata);
