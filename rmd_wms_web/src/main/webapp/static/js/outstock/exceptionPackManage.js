...
var newurl=basepath + '/outstock/ListPickBiLL?flag=2';
$(function() {
	// 拣货计划
	init(newurl);
...
function init(url) {
	...
	var queryParams = {};
	$('#exceptionPackListSearch').find('*').each(function() {
		queryParams[$(this).attr('name')] = $(this).val();
	});
	if ($("#orderEx").is(":checked")) {
		queryParams['orderEx']="4";
	}
	if ($("#packEx").is(":checked")) {
		queryParams['packEx']="0";
	}
	var pageNo = 1;
	var rows = 10;
...
//搜索事件
function exceptionPackListSearchBox(){
	init(newurl);
}
...
//Excel导出
$("#exceptionPackExport").click(function(){
	//模拟表单提交
	...
	input1.attr("value",JSON.stringify(exceptionPackExcelHead));
	var input2=$("<input>");
