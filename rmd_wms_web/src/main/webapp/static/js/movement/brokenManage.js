...
var newurl = basepath + '/movement/ListMovement?flag=1';
$(function() {
	// 出库制单
	init(newurl);
...
function init(url) {
	...
	var queryParams = {};
	$('#brokenMoveListSearch').find('*').each(function() {
	...
	$('#brokenmove_list').datagrid({
		...
		// 	align : 'center',
		// 	formatter : function(value, row, index) {
		// 		return formatDatebox(row.movementInfo.inStockTime);
		// 	}
		}, {
			field : 'validityTime',
			title : '有效期',
		...
		onExpandRow: function(index,row){
			...
			ddv.datagrid({
				...
				onResize:function(){
					$('#brokenmove_list').datagrid('fixDetailRowHeight',index);
				},
...
$("#brokenExport").click(function(){
	...
	input1.attr("value",JSON.stringify(brokenExcelHead));
